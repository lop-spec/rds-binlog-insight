...
const MONGO_METRICS = {CPUUtilization:'CPU 使用率 %',MemoryUtilization:'内存使用率 %',ConnectionAmount:'连接数',IOPSUtilization:'IOPS 使用率 %',ReplicationLatency:'复制延迟 s',QPS:'QPS',ScannedDocs:'扫描文档 / 秒',ScannedKeys:'扫描索引 / 秒',ReadIops:'读 IOPS',WriteIops:'写 IOPS',AvgRt:'平均响应 μs',ReadAvgRt:'读取平均响应',WriteAvgRt:'写入平均响应',WtCacheUsage:'WT 缓存使用率 %',WtCacheDirtyUsage:'WT 脏页使用率 %',ConcurrentReads:'读等待',ConcurrentWrites:'写等待',CentralCacheFree:'TCMalloc 中央空闲',TcmallocCacheMemRatio:'TCMalloc 内存碎片率'};
...
async function runMongoAnalytics() {
  ...
  const start=new Date($('#analytics-start').value).getTime()*1000,end=new Date($('#analytics-end').value).getTime()*1000;
...
function renderMongoAnalytics(data) {
  ...
  const counterRows=(data.native_counters||[]).map(x=>[escapeHtml(x.node),escapeHtml(x.role),escapeHtml(x.command),mongoNumber(x.count),mongoNumber(x.qps),`${mongoNumber(x.coverage_seconds)} / ${mongoNumber(x.window_seconds)} 秒`,mongoNumber(x.failed)]);
  const memory=(data.native_latest||[]).map(x=>{
    const g=x.tcmalloc?.generic||{},c=x.wt_cache||{},free=x.tcmalloc?.tcmalloc_derived||{};
    return [escapeHtml(x.node),escapeHtml(x.role),formatTime(x.timestamp*1000),mongoNumber(x.mem?.resident/1024),mongoNumber(c['bytes currently in the cache']/2**30),mongoNumber(c['maximum bytes configured']/2**30),mongoNumber(g.current_allocated_bytes/2**30),mongoNumber(free.total_free_bytes/2**30),mongoNumber(x.connections?.current)];
  });
  ...
  $('#analytics-panel-sql').innerHTML=`<section class="detail-block"><h3>慢命令增量与性能关联</h3><p>${totals}</p><p class="analytics-note">${escapeHtml(data.warning)} · 只对所选层级计算。缺 CPU/读取字段显示未知，不补零。</p>
    ...
    </section><details class="detail-block"><summary>最长慢命令与反证（独立于增量榜）</summary>${analyticsTable(['命令','最长耗时','代表样本开始','排除项'],outlierRows)}</details><details class="detail-block"><summary>节点命令总次数（不是慢日志计数）</summary><p>按服务器原生计数器的连续区间相减；缺失和跨进程区间不计。QPS 使用已覆盖秒数，不外推完整窗口。</p>${analyticsTable(['节点','角色','命令','计数增量','QPS','覆盖 / 窗口','失败'],counterRows,'该历史窗口未采集原生命令计数，不能从慢日志补出来。')}${detailBlock('计数缺口',JSON.stringify(data.native_gaps||[]))}</details>
    <details class="detail-block"><summary>内存组成与当前节点状态</summary>${analyticsTable(['节点','角色','样本时间','RSS GiB','WT GiB','WT 上限 GiB','实际分配 GiB','分配器空闲 GiB','连接'],memory,'该窗口无原生内存快照。')}</details>
