...
class ClickHouseGates(unittest.TestCase):
    def test_serving_exactness_and_loss_detection(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            native=dict(timestamp=T//1000,node='fixture-node',role='Primary',interval={'status':'ok','commands':{'find':{'total':2}}})
            store.telemetry(instance,'native',[native])
            self.assertEqual(store.read_telemetry(instance,'native',T,T+MINUTE,compact=True)[0]['interval']['commands']['find']['total'],2)
