...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.9.3-pre-v1.9.4"
RESULT_FILE = ROOT / "deploy-v1.9.4-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v194.tmp")
...
""".strip()
    ...
    if (
        int(value["analyticsFormatVersion"]) != 2
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.9.4-staging").resolve()
