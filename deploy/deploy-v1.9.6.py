...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.9.5-pre-v1.9.6"
RESULT_FILE = ROOT / "deploy-v1.9.6-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v196.tmp")
...
""".strip()
    ...
    if (
        int(value["analyticsFormatVersion"]) != 3
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.9.6-staging").resolve()
