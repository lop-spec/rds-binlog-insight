...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.10.0-pre-v1.10.1"
RESULT_FILE = ROOT / "deploy-v1.10.1-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def run(
    ...
    *,
    ...
    `check=True` 抛出的 CalledProcessError 只有命令行和退出码，实际 stderr 会
    丢失，导致部署失败后无法定位。这里显式保留最后 4000 字符输出。
    `retries` 用于 docker compose 这类会被瞬时容器状态竞态影响的命令。
    """
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v1101.tmp")
...
def verify_candidate(staging: Path) -> None:
    ...
    if (
        ...
        or "catalog_relevant_parts" not in exact_index
        or "indices = self._registered_indices(selector)" not in exact_index
        or "columns=[\"database_name\", \"table_name\"]" not in exact_index
        or "catalog_relevant_parts" not in (staging / "app" / "index_worker.py").read_text(encoding="utf-8")
        or "EXACT_INDEX_WORKERS = 4" not in (staging / "app" / "index_worker.py").read_text(encoding="utf-8")
        or 'thread_name_prefix="exact-index-worker"' not in (staging / "app" / "index_worker.py").read_text(encoding="utf-8")
        or "self.exact_index.lookup" not in storage
        or "self.exact_index.build_segment([(current, source)])" not in storage
...
from app.analytics_index import ANALYTICS_FORMAT_VERSION, BUCKET_US
from app.config import APP_VERSION
from app.index_worker import ANALYTICS_BATCH_SIZE, ANALYTICS_WORKERS
from app.sql_fingerprint import (
...
from app.index_worker import EXACT_INDEX_BATCH_SIZE, EXACT_INDEX_WORKERS, INDEX_BATCH_SIZE
...
""".strip()
    ...
    if (
        int(value["exactIndexBatchSize"]) != 4
        or int(value["exactIndexWorkers"]) != 4
        or int(value["exactRegistryMappings"]) != 1
        or int(value["exactRegisteredTables"]) != 1
    ):
        raise RuntimeError(f"candidate exact-index probe failed: {value}")
    if (
        int(value["analyticsFormatVersion"]) != 4
        or int(value["analyticsFingerprintVersion"]) != 2
        or int(value["analyticsBucketUs"]) != 5 * 60 * 1_000_000
        or int(value["analyticsBatchSize"]) != 2
        or int(value["analyticsWorkers"]) != 2
    ):
        raise RuntimeError(f"candidate analytics constant probe failed: {value}")
    if value["analyticsStatsKeys"] != [
        "degraded_parts",
        "index_bytes",
        "max_event_epoch_us",
        "min_event_epoch_us",
        ...
        raise RuntimeError(f"candidate analytics index probe failed: {value}")
    # 归一化必须真的把字面量参数化并折叠 IN 列表，否则同一条语句会按参数分裂。
    if value["analyticsNormalizes"] != "UPDATE t SET a=? WHERE id IN (?)":
        raise RuntimeError(f"candidate SQL fingerprint probe failed: {value}")
    # `@1` 是列序号不是字面量；参数化它会把不同列的更新错误归并。
    if "@1" not in str(value["analyticsOrdinalKept"]):
        raise RuntimeError(f"candidate column ordinal probe failed: {value}")
    # 重建 SQL 与事务边界绝不能冒充 QueryEvent 的真实语句文本。最后两项锁定
    # MySQL 把 BEGIN/COMMIT 写成普通 QueryEvent 的情况——漏判会让它们按事务
    # 数量级霸占语句榜单。
    if value["analyticsSources"] != [
        "reconstructed",
        "boundary",
        "original",
        "synthetic",
        ...
        raise RuntimeError(f"candidate statement source probe failed: {value}")
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.10.1-staging").resolve()
    ...
    try:
        ...
        run(
            ...
            cwd=ROOT,
            # `docker rm` 返回后守护进程可能仍在回收容器名与网络端点，紧接着的
            # up 会瞬时失败。重试两次即可，真实故障仍会带完整输出抛出。
            retries=2,
        )
        activated = True
