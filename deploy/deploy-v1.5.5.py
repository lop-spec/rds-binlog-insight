...
NEW_IMAGE = "local/rds-binlog-insight:1.5.5"
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.5.4-pre-v1.5.5"
RESULT_FILE = ROOT / "deploy-v1.5.5-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v155.tmp")
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.5.5-staging").resolve()
    ...
    try:
        ...
        activated = True
        current_health = wait_health("1.5.5")
