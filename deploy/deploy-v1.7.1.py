...
NEW_IMAGE = "local/rds-binlog-insight:1.7.1"
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.7.0-pre-v1.7.1"
RESULT_FILE = ROOT / "deploy-v1.7.1-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v171.tmp")
...
def verify_candidate(staging: Path) -> None:
    ...
    if (
        "FILE_PIPELINE_WORKERS = 3" not in pipeline
        or "DOWNLOAD_PIPELINE_WORKERS = 3" not in pipeline
        or "TRANSFORM_PIPELINE_WORKERS = 3" not in pipeline
        ...
        raise RuntimeError(
            "candidate would remove the process-isolated bounded 3-lane pipeline"
        )
...
""".strip()
    ...
    if (
        int(value["fileWorkers"]) != 3
        or int(value["downloadWorkers"]) != 3
        or int(value["transformWorkers"]) != 3
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.7.1-staging").resolve()
    ...
    try:
        ...
        activated = True
        current_health = wait_health("1.7.1")
        ...
        if (
            int(pipeline_status.get("fileWorkers") or 0) != 3
            or int(pipeline_status.get("downloadWorkers") or 0) != 3
            or int(pipeline_status.get("transformWorkers") or 0) != 3
