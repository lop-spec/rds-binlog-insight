...
CURRENT_VERSION = "1.8.29"
CURRENT_IMAGE = f"local/rds-binlog-insight:{CURRENT_VERSION}"
NEW_VERSION = "1.8.30"
NEW_IMAGE = f"local/rds-binlog-insight:{NEW_VERSION}"
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.8.29-pre-v1.8.30"
RESULT_FILE = ROOT / "deploy-v1.8.30-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v1830.tmp")
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.8.30-staging").resolve()
