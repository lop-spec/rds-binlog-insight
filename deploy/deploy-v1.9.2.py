...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.9.1-pre-v1.9.2"
RESULT_FILE = ROOT / "deploy-v1.9.2-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v192.tmp")
...
""".strip()
    ...
    # 重建 SQL 与事务边界绝不能冒充 QueryEvent 的真实语句文本。
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.9.2-staging").resolve()
