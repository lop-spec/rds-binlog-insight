...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.9.2-pre-v1.9.3"
RESULT_FILE = ROOT / "deploy-v1.9.3-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v193.tmp")
...
""".strip()
    ...
    if (
        ...
        or int(value["analyticsFingerprintVersion"]) != 1
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.9.3-staging").resolve()
