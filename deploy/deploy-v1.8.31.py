...
CURRENT_VERSION = "1.8.30"
CURRENT_IMAGE = f"local/rds-binlog-insight:{CURRENT_VERSION}"
NEW_VERSION = "1.8.31"
NEW_IMAGE = f"local/rds-binlog-insight:{NEW_VERSION}"
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.8.30-pre-v1.8.31"
RESULT_FILE = ROOT / "deploy-v1.8.31-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v1831.tmp")
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.8.31-staging").resolve()
