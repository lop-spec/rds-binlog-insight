...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.8.28-pre-v1.8.29"
RESULT_FILE = ROOT / "deploy-v1.8.29-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v1829.tmp")
...
def verify_candidate(staging: Path) -> None:
    ...
    if (
        ...
        or "table, row_offsets = self._registered_rows(table)" not in exact_index
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.8.29-staging").resolve()
