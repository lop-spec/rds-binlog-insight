...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.9.0-pre-v1.9.1"
RESULT_FILE = ROOT / "deploy-v1.9.1-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v191.tmp")
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.9.1-staging").resolve()
