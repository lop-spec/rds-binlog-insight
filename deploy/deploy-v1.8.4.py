...
NEW_IMAGE = "local/rds-binlog-insight:1.8.4"
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.8.3-pre-v1.8.4"
RESULT_FILE = ROOT / "deploy-v1.8.4-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v172.tmp")
...
def verify_candidate(staging: Path) -> None:
    ...
    if (
        "app.index_supervisor" not in compose
...
from app.index_worker import INDEX_BATCH_SIZE
...
print(json.dumps({"sqlite": pysqlite3.sqlite_version, "pyarrow": pyarrow.__version__, "duckdb": duckdb.__version__, "ftsDeleteRemaining": remaining, "fileWorkers": FILE_PIPELINE_WORKERS, "downloadWorkers": DOWNLOAD_PIPELINE_WORKERS, "transformWorkers": TRANSFORM_PIPELINE_WORKERS, "archiveWorkers": OSS_ARCHIVE_WORKERS, "archiveBacklogPerFile": OSS_ARCHIVE_BACKLOG_PER_FILE, "freshUploadTargetBytes": OSS_FRESH_UPLOAD_TARGET_BYTES, "indexBatchSize": INDEX_BATCH_SIZE, "packTargetBytes": OSS_PACK_TARGET_BYTES, "localIndexHandoffBytes": LOCAL_INDEX_HANDOFF_MAX_BYTES, "nativeChunkMaxBytes": NATIVE_CHUNK_MAX_BYTES, "nativeChunkMaxOutstanding": NATIVE_CHUNK_MAX_OUTSTANDING, "nativeStagingBoundBytes": NATIVE_CHUNK_MAX_BYTES * NATIVE_CHUNK_MAX_OUTSTANDING, "duckdbThreads": duck_threads, "packRange": pack_range, "packColumns": sorted({"oss_offset", "oss_length", "oss_object_sha256"} & part_columns), "duckdbSpillUnderData": spill.is_relative_to((data_root / "scratch").resolve()), "bodyLocksUnderData": storage.paths["locks"].resolve().is_relative_to(data_root.resolve())}))
""".strip()
    value = json.loads(
        run(
            [
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.8.4-staging").resolve()
    ...
    try:
        ...
        activated = True
        current_health = wait_health("1.8.4")
        ...
        for label, inspected in (
            ...
            if int(inspected["HostConfig"].get("NanoCpus") or 0) != 1_000_000_000:
                raise RuntimeError(f"{label} one-CPU limit was not applied")
