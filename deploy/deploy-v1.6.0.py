...
NEW_IMAGE = "local/rds-binlog-insight:1.6.0"
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.5.5-pre-v1.6.0"
RESULT_FILE = ROOT / "deploy-v1.6.0-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def verify_candidate(staging: Path) -> None:
    ...
    if (
        ...
        or "OSS_ARCHIVE_WORKERS = 3" not in pipeline
        or "def _run_pending_parallel" not in pipeline
    ):
        raise RuntimeError("candidate would remove the bounded 3-lane pipeline")
...
c.close()
print(json.dumps({"sqlite": pysqlite3.sqlite_version, "pyarrow": pyarrow.__version__, "duckdb": duckdb.__version__, "ftsDeleteRemaining": remaining, "fileWorkers": FILE_PIPELINE_WORKERS, "downloadWorkers": DOWNLOAD_PIPELINE_WORKERS, "archiveWorkers": OSS_ARCHIVE_WORKERS}))
""".strip()
    value = json.loads(
        run(
            [
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.6.0-staging").resolve()
    ...
    try:
        ...
        activated = True
        current_health = wait_health("1.6.0")
        ...
        if (
            ...
            or int(pipeline_status.get("archiveWorkers") or 0) != 3
        ):
            raise RuntimeError(
