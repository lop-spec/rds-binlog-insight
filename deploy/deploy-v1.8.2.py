...
NEW_IMAGE = "local/rds-binlog-insight:1.8.2"
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.8.1-pre-v1.8.2"
RESULT_FILE = ROOT / "deploy-v1.8.2-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def verify_candidate(staging: Path) -> None:
    ...
    if (
        ...
        raise RuntimeError(
            "candidate would remove the fast ZSTD/direct OSS hot path"
        )
...
print(json.dumps({"sqlite": pysqlite3.sqlite_version, "pyarrow": pyarrow.__version__, "duckdb": duckdb.__version__, "ftsDeleteRemaining": remaining, "fileWorkers": FILE_PIPELINE_WORKERS, "downloadWorkers": DOWNLOAD_PIPELINE_WORKERS, "transformWorkers": TRANSFORM_PIPELINE_WORKERS, "archiveWorkers": OSS_ARCHIVE_WORKERS, "archiveBacklogPerFile": OSS_ARCHIVE_BACKLOG_PER_FILE, "freshUploadTargetBytes": OSS_FRESH_UPLOAD_TARGET_BYTES, "indexBatchSize": INDEX_BATCH_SIZE, "packTargetBytes": OSS_PACK_TARGET_BYTES, "localIndexHandoffBytes": LOCAL_INDEX_HANDOFF_MAX_BYTES, "duckdbThreads": duck_threads, "packRange": pack_range, "packColumns": sorted({"oss_offset", "oss_length", "oss_object_sha256"} & part_columns), "duckdbSpillUnderData": spill.is_relative_to((data_root / "scratch").resolve())}))
""".strip()
    value = json.loads(
        run(
            [
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.8.2-staging").resolve()
    ...
    try:
        ...
        activated = True
        current_health = wait_health("1.8.2")
