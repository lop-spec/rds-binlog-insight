...
BACKUP_IMAGE = "local/rds-binlog-insight:backup-v1.8.31-pre-v1.9.0"
RESULT_FILE = ROOT / "deploy-v1.9.0-result.json"
SOURCE_FILES = (
    ".dockerignore",
    ".env.example",
    "Dockerfile",
...
def atomic_copy(source: Path, target: Path, mode: int | None = None) -> None:
    ...
    temporary = target.with_name(f".{target.name}.deploy-v190.tmp")
...
from app.sql_fingerprint import FINGERPRINT_FORMAT_VERSION, normalize_sql
...
""".strip()
    ...
    if (
        int(value["analyticsFormatVersion"]) != 1
...
def main() -> int:
    ...
    staging = (ROOT / ".deploy-v1.9.0-staging").resolve()
