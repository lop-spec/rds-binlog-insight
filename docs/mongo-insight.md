...
- If a growth ranking has no usable comparison, the API explicitly returns `requested_order`, effective `order` and `order_reason`, and logs the change. It ranks measured cost (or count if that field is absent), not an arbitrary fingerprint hash. The UI discloses this change without silently changing the query window. Current-window correlation needs complete current source/metric coverage, not a historical baseline.
...
- Supported orders: `duration_growth`, `count_growth`, `scan_growth`, `cpu_growth`, `read_growth`, `performance`, `count`, `max_latency`, `duration_total`, `scan_total`, `cpu_total`, `read_total`.
