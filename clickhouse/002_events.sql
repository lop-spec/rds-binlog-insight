...
-- CREATE TABLE IF NOT EXISTS does not update settings on an existing hot
-- table.  Keep the migration idempotent and cap one automatic merge at 128 MiB
-- so SYSTEM STOP MERGES converges quickly on a shared disk.
ALTER TABLE insight.events MODIFY SETTING
    max_bytes_to_merge_at_max_space_in_pool = 134217728;
