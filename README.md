...
- [MongoDB 慢命令、原生命令计数与性能关联](docs/mongo-insight.md)：在同一分析工作台复用存储和关联计算；区分命令/内部操作、慢记录/全量计数、成本增长/单次退化，保留最长慢记录的反证。Mongo 配置缺失时不改变 MySQL 路径；集合/模板全量次数仅覆盖实际接入的应用服务。
...
- 下载、解析/Parquet 与 OSS 归档使用独立有界执行池。默认最多两个文件处理通道、
  ...
  为 2 CPU、索引服务为 1 CPU，实际部署以生效配置及父 cgroup 为准。
  下载线程为 3 个，默认最多准入 4 个文件（含正在解析、已下载和下载中的文件），
...
- 自动任务每 5 分钟检查 60 天窗口；发现器兼顾新文件与保留期内历史文件，
  不丢弃未核实的历史记录，按准入顺序开放查询和提交。
- 面板吞吐按同节点、同日志类型的最近两小时墙钟窗口计算文件/小时与原始字节/小时，
  不使用单文件耗时倒数。库存清空估算不含新增文件；源端库存仅是生成速率下界，
  未证明源端覆盖完整时不报告持续净追平时间，失败或不可用文件不视为完成。
...
`downloads/` 的活跃准入默认最多 4 个文件、按源文件大小合计 2 GiB；超大单文件
独占准入。中断遗留的可恢复文件也需要额外磁盘空间，不能把活跃配额当作目录总量上限。
文件按顺序提交后删除。`staging/` 每通道按 3 × 128 MiB 预留转换、预取及
写入中的批次，双通道总预算 768 MiB，另受默认 Compose 的 1 GiB 暂存盘硬上限约束。
新 Parquet 独立直传 OSS，历史迁移兼容约 128 MiB 聚合对象；每文件最多
5 个上传任务后背压。DuckDB 溢写继续使用 `/data/scratch` 的 NVMe，不占 RAM 暂存盘。
...
## 工作台与慢 SQL 相关度（v1.27.0）

审计与分析页只常驻主要范围条件；其余条件集中在“更多筛选”，已启用条件在折叠后仍显示摘要。
时间范围和 SQL 排序各用一个选择器，管理页面归入“管理”。慢 SQL 主表保留 7 列，
SQL ID、单次扫描、扫描/返回比与详细耗时放入 SQL 明细；次要指标和分布按需展开。

“相关度 r”使用同一筛选范围、同一去重快照下的每桶执行次数：
`x = 该 SQL 指纹次数`，`y = 全部慢 SQL 次数（不是 Top N 合计，包含自身）`，
主指标为 `Pearson(Δx, Δy)`。按趋势粒度对齐并补齐零记录桶，不累计、不平滑、不搜索时滞；
不完整首尾桶只展示、不参与系数，少于 6 个完整桶或增量零方差返回空值及原因。
整数充分统计量避免大基数消减误差，只在最终开方和除法使用浮点数。
明细提供次数相关、剔除自身后的增量相关、次数占比、活跃桶数和逐桶计数，便于复算。

相关度仅描述已采集日志，不证明因果或增长贡献；索引覆盖完整不代表云端采集完整。
少量样本、低频尖峰、筛选和粒度都会影响解释，6 桶门槛不代表统计显著性。
SQLite 与 ClickHouse 共用计算实现；ClickHouse 在原聚合内用 `sumMap` 生成时序，
不增加逐 SQL 查询，也不通过第二次扫描拼接可能已经变化的总体曲线。

验证：`python -m unittest tests.test_slowlog_correlation tests.test_slowlog_correlation_engines`
以及 `node --test tests/test_workspace.cjs`。本次不涉及数据库 schema 变更。
...
`1.26.11-rawoss` 修复慢日志索引器的写锁长持有：`slowlog_event_details` 按
`(event_id, part_path)` 主键删除，不再在 `BEGIN IMMEDIATE` 事务内全表扫描（生产实测
每分区 1.5 GB 读、2 分钟，采集入队因此 `database is locked`）；采集器入队遇锁有界
重试并留痕；ClickHouse 慢日志通道不再因源端 reconcile 扫描进行中而暂停，只看真实积压。
