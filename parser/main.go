...
type outputEvent struct {
	...
	TxnLengthBytes    uint64 `json:"txn_length_bytes"`
}
...
type extractor struct {
	...
	txnLengthBytes     uint64
	lastEpochUS        int64
	requireGTID        bool
	requireTableMap    bool
...
func headerEpochUS(header *replication.EventHeader) int64 {
	if header.Timestamp == 0 {
		return 0
	}
	return int64(header.Timestamp) * 1_000_000
}
...
func schemaVersionID(databaseName, tableName, columnsJSON string) string {
	stable := strings.ToLower(databaseName) + "\x00" + strings.ToLower(tableName) + "\x00" + columnsJSON
	sum := sha256.Sum256([]byte(stable))
...
func binaryColumnSet(table *replication.TableMapEvent) map[int]bool {
	result := make(map[int]bool)
	for index, collation := range table.CollationMap() {
		// MySQL collation 63 is the binary pseudo-charset used by BINARY,
		// VARBINARY and binary string/blob columns.
		if collation == 63 {
			result[index] = true
		}
	}
...
func normalizeColumnValue(value any, column int, binaryColumns map[int]bool) any {
	if text, ok := value.(string); ok && (binaryColumns[column] || !utf8.ValidString(text)) {
		return normalizeValue([]byte(text))
	}
	return normalizeValue(value)
}

func rowObject(row []any, skipped []int, names []string, binaryColumns map[int]bool) map[string]any {
	...
	for index := range row {
		...
		result[name] = normalizeColumnValue(row[index], index, binaryColumns)
	}
	return result
}
...
func pseudoLiteral(value any) string {
	...
	case map[string]any:
		if encoded, ok := item["$binary_base64"].(string); ok {
			return "FROM_BASE64('" + strings.ReplaceAll(encoded, "'", "''") + "')"
		}
		if encoded, ok := item["$bytes_base64"].(string); ok {
			return "FROM_BASE64('" + strings.ReplaceAll(encoded, "'", "''") + "')"
		}
...
func (x *extractor) handleRows(
	...
	schemaColumnsJSON, err := compactJSON(descriptions)
	if err != nil {
		return fmt.Errorf("encode column metadata: %w", err)
	}
	columnsJSON := schemaColumnsJSON
	if x.slim {
		columnsJSON = ""
	}
	schemaID := schemaVersionID(databaseName, tableName, schemaColumnsJSON)
	binaryColumns := binaryColumnSet(event.Table)
	...
	for index := 0; index < len(event.Rows); {
		...
		if operation == "UPDATE" {
			before = rowObject(event.Rows[index], skippedAt(index), names, binaryColumns)
			after = rowObject(event.Rows[index+1], skippedAt(index+1), names, binaryColumns)
			index += 2
		} else if operation == "INSERT" {
			after = rowObject(event.Rows[index], skippedAt(index), names, binaryColumns)
			index++
		} else {
			before = rowObject(event.Rows[index], skippedAt(index), names, binaryColumns)
			index++
		...
		if err := x.emit(outputEvent{
			...
			RowQuery:          rowQuery,
			HeaderEpochUS:     headerEpochUS(binlogEvent.Header),
			CommitEpochUS:     x.transactionEpochUS,
...
func (x *extractor) handleEvent(
	...
	case *replication.GTIDEvent:
		...
		x.txnLengthBytes = event.TransactionLength
	...
	case *replication.QueryEvent:
		...
		if err := x.emit(outputEvent{
			...
			HeaderEpochUS:     headerEpochUS(header),
			CommitEpochUS:     x.transactionEpochUS,
			TxnLastCommitted:  x.txnLastCommitted,
			TxnSequenceNumber: x.txnSequenceNumber,
			TxnLengthBytes:    x.txnLengthBytes,
...
type parserCoverage struct {
	...
	sawGTID      bool
	sawTableMap  bool
	tableMaps    map[uint64]struct{}
}

func readBinlogEvent(reader io.Reader, offset uint64) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("partial binlog event header at offset %d: read %d of %d bytes: %w", offset, read, binlogEventHeaderSize, err)
	}
	eventSize := binary.LittleEndian.Uint32(header[binlogEventSizeOffset : binlogEventSizeOffset+4])
	if eventSize < binlogEventHeaderSize || eventSize > maxBinlogEventSize {
		return nil, false, fmt.Errorf("invalid binlog event size %d at offset %d", eventSize, offset)
	}
	expectedEnd := offset + uint64(eventSize)
	if expectedEnd > math.MaxUint32 {
		return nil, false, fmt.Errorf("binlog position exceeds uint32 at offset %d", offset)
	}
	logPosition := binary.LittleEndian.Uint32(header[13:17])
	if uint64(logPosition) != expectedEnd {
		return nil, false, fmt.Errorf("binlog event position mismatch at offset %d: header=%d expected=%d", offset, logPosition, expectedEnd)
	}
	raw := make([]byte, int(eventSize))
	copy(raw, header)
	if _, err := io.ReadFull(reader, raw[binlogEventHeaderSize:]); err != nil {
		return nil, false, fmt.Errorf("partial binlog event body at offset %d: size %d: %w", offset, eventSize, err)
	}
	return raw, false, nil
}

func inspectDecodedEvent(binlogEvent *replication.BinlogEvent, coverage *parserCoverage) error {
	switch event := binlogEvent.Event.(type) {
	case *replication.GTIDEvent, *replication.MariadbGTIDEvent:
		coverage.sawGTID = true
	case *replication.TableMapEvent:
		coverage.sawTableMap = true
		coverage.tableMaps[event.TableID] = struct{}{}
	case *replication.RowsEvent:
		if event.Table == nil {
			return fmt.Errorf("rows event at position %d has no TableMap context", binlogEvent.Header.LogPos)
		}
		if _, ok := coverage.tableMaps[event.TableID]; !ok {
			return fmt.Errorf("rows event at position %d references unseen TableMap ID %d", binlogEvent.Header.LogPos, event.TableID)
		}
	case *replication.TransactionPayloadEvent:
		for _, child := range event.Events {
			if err := inspectDecodedEvent(child, coverage); err != nil {
				return err
			}
		}
	}
...
func (x *extractor) parseBinlog(reader io.Reader) error {
	...
	first := true
	for {
		raw, eof, err := readBinlogEvent(reader, offset)
		if err != nil {
			return err
		}
		if eof {
		...
		coverage.framedEvents++
		if first {
			raw, err = normalizeFormatDescriptionEvent(raw)
			if err != nil {
				return fmt.Errorf("prepare MySQL binlog: %w", err)
			}
			first = false
		} else if raw[4] == formatDescriptionEventType {
			return fmt.Errorf("unexpected duplicate FormatDescriptionEvent at offset %d", offset)
		}
		eventType := replication.EventType(raw[4])
		if eventType.String() == "UnknownEvent" {
			return fmt.Errorf("unknown binlog event type %d at offset %d", raw[4], offset)
		}
		binlogEvent, err := parser.Parse(raw)
		if err != nil {
			return fmt.Errorf("parse MySQL binlog event %s at offset %d: %w", eventType.String(), offset, err)
		}
		if binlogEvent == nil || binlogEvent.Header == nil || binlogEvent.Event == nil {
			return fmt.Errorf("parser returned incomplete event %s at offset %d", eventType.String(), offset)
		}
		coverage.parsedEvents++
		if err := inspectDecodedEvent(binlogEvent, &coverage); err != nil {
			return err
		}
		if err := x.handleEvent(binlogEvent, nil, nil); err != nil {
			return err
		}
		offset += uint64(len(raw))
	}
	if first {
		return errors.New("missing FormatDescriptionEvent")
	}
	if coverage.framedEvents != coverage.parsedEvents {
		return fmt.Errorf("binlog event coverage mismatch: framed=%d parsed=%d", coverage.framedEvents, coverage.parsedEvents)
	}
	if x.requireGTID && !coverage.sawGTID {
		return errors.New("required GTID context is missing")
	}
	if x.requireTableMap && !coverage.sawTableMap {
		return errors.New("required TableMap context is missing")
	}
	return nil
}
